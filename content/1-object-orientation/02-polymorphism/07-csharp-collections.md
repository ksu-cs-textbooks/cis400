...
Collections in C# are a great example of polymorphism in action.  Many collections utilize [generics]({{% relref "1-object-orientation/06-advanced-csharp/05-generics" %}}) to allow the collection to hold an arbitrary type.  For example, the `List<T>` can be used to hold strings, integers, or even specific objects:
...
We can also use an interface as the type, as we did with the `IJumpable` interface as we discussed in the [generics section]({{% relref "1-object-orientation/06-advanced-csharp/05-generics" %}}), i.e.:
